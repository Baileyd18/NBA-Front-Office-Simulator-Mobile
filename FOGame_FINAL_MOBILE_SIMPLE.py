...
with st.popover(f"📋 View Roster ({len(roster_rows_mobile)}/13)", use_container_width=True):
    st.write(f"**Payroll:** {money(payroll_mobile)}")
    st.write(f"**Remaining:** {money(remaining_mobile)}")

    if not roster_rows_mobile:
        ...
        for slot in ROSTER_SLOTS:
            if slot in st.session_state.roster:
                ...
                st.markdown(
                    f"**{slot}:** {p['Player']}  \n"
                    f"{p['Pos']} | {money(p['Salary'])} | "
                    f"{float(p['PTS']):.1f} PPG / {float(p['AST']):.1f} APG / {float(p['TRB']):.1f} RPG"
                )
            else:
                st.caption(f"{slot}: Empty")
